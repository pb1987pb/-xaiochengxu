...
this.requestData(this.cid,this.proname);
  ...
  methods:{
   ...
   async requestData(cid,proname){
               var res=  await this.$ajax('/API/WXGetData/worked'+'?cid='+cid);
        this.childdata=res.data;
          this.childdata.proname=proname;
          console.log(this.childdata);
            },
            // 有审批权限的人
            async yesrequestData(cid,proname,wtid){
        ...
        this.childdata.icon="icon-baoxiao";
          this.childdata.proname=proname;
            },async yibantongguo(cid,str){ //一般项目通过

               var   res= await this.$ajax('/API/WXGetData/GeneralExpensePass'+'?eid='+cid+'&remark='+str);
            this.chuli(res);

            },async yibanjujue(cid,str){//一般项目拒绝

               var res=  await this.$ajax('/API/WXGetData/GeneralExpenseReject'+'?eid='+cid+'&remark='+str);
                     this.chuli(res);

            },async xiangmutongguo(cid,str){ //项目通过   
               var res=  await this.$ajax('/API/WXGetData/ProjectExpensePass'+'?eid='+cid+'&remark='+str);
                 this.chuli(res);

            },async xiangmujujue(cid,str){ //项目拒绝

               var res=  await this.$ajax('/API/WXGetData/ProjectExpenseReject'+'?eid='+cid+'&remark='+remark);
  ...
  success: function(res) {
    ...
    if (res.confirm) {//通过
        ...
        if(that.fenlei==1){ //项目报销
         
          that.xiangmutongguo(that.cid,str);

        }else if(that.fenlei==2){ //一般报销
           that.yibantongguo(that.cid,str,);
        }
  ...
  success: function(res) {
    if (res.confirm) {
         if(that.fenlei==1){ //项目报销
             that.xiangmujujue(that.cid,str);
        }else if(that.fenlei==2){ //一般报销
           that.yibanjujue(that.cid,str);
        }
    } else if (res.cancel) {
      console.log('用户点击取消')
    }
