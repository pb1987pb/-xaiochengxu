    ...
    <view>
      <view class="mes">
        <view>申请人：<text class="col">{{childdata.NickName}}</text></view>
           ...
           <view>
             <!--<view>项目收款进度：<text class="col">40%</text></view>-->
              <view>报销金额：<text class="col">￥{{childdata.SumMoney}}</text></view>
             <view>未报销金额：<text class="col">￥{{childdata.NotSubmitMoney}}</text></view>
            ...
            <view class="bbox">报销底单：<a :href="'/pages/didan/main?eid='+childdata.ID" class="didan">点击查看</a></view>
            <view class="bbox">报销单：<a :href="'/pages/bxdidan/main?eid='+childdata.ID" class="didan">点击查看</a></view>
           </view>
      ...
      <view class="fujian bbox">
          ...
          <view class="rf" v-if="childdata.arrayo">
            <view v-for="(item, itemIndex) in childdata.arrayo" :key="itemIndex"  @click="download(item.fileurl,item.filetype)">
                   {{item.filename}}
              </view>
          </view>
      </view>
