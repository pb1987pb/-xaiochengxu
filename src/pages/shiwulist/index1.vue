...
import shiwulist from '@/components/shiwulist'
...
this.pageindex++;
   var  res= await this.$ajax('/API/WXGetData/TranWorked'+'?searchwords='+this.searword+'&pageindex='+this.pageindex+'&pagesize='+this.pagesize);
...
async shaixuan(str){ //点击子组件的待办和已办
     ...
     res= await this.$ajax('/API/WXGetData/TranWait');
     }else{
       res= await this.$ajax('/API/WXGetData/TranWorked'+'?searchwords='+this.searword+'&pageindex='+this.pageindex+'&pagesize='+this.pagesize);
