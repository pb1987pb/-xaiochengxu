    ...
    <view>
      <view class="mes">
        <view>申请人：<text class="col">{{childdata.NickName}}</text></view>
         <view>
           <view v-if="PaymentStatus">项目收款进度：<text class="col">{{childdata.PaymentStatus}}%</text></view>
             <view>付款总金额：<text class="col">￥{{childdata.moy}}</text></view>
           <view v-if="CurBudget">项目剩余预算：<text class="col">￥{{childdata.CurBudget}}</text></view>
           <view>收款单位：<text class="col">{{childdata.PayyeeCompany}}</text></view>
           <view>银行卡号：<text class="col">{{childdata.BankAccount}}</text></view>
           <view>联系人：<text class="col">{{childdata.LinkPerson}}</text></view>
           <view>联系电话：<text class="col">{{childdata.LinkMobile}}</text></view>
            <view>合同总价格：<text class="col">{{childdata.ContractTotal}}</text></view>
             <view>总预算额度：<text class="col">{{childdata.TotalBudget}}</text></view>
              <view>当时预算花费：<text class="col">{{childdata.CurProjectCost}}</text></view>
               <view>付款进度：<text class="col">{{childdata.PayStatus}}%</text></view>
...
.bxxq_page .boxwrap .type{width: 50%;}
</style>