...
this.pageindex++;
   var  res= await this.$ajax('/API/WXGetData/BorrowForLonComplete'+'?searchwords='+this.searword+'&pageindex='+this.pageindex+'&pagesize='+this.pagesize);
...
this.backtop(0);
     ...
     var  res= await this.$ajax('/API/WXGetData/BorrowForLonIndex');
    ...
    if(res.Res==1&&(res.data.expenseList.length>0||res.data.waitExpenseList.length>0))
           {
              ...
              this.dataobj={
    'href':'/pages/jiekuandetail/main',
    'icon':'icon-jiekuanshenqing',
    'arr':res.data,
    'type':this.type
        }
      }else{
       ...
       var  res= await this.$ajax('/API/WXGetData/BorrowForLonComplete'+'?searchwords='+this.searword);
console.log(res);
