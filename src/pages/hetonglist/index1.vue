...
//  },500);
if(!this.flag){
this.flag=this.debounce(this.queryMultipleNodes,500);
}
this.flag();
  // 限流，前一次没执行完，后一次不会执行
...
this.pageindex++;
   var  res= await this.$ajax('/API/WXGetData/ConTraYcl'+'?searchwords='+this.searword+'&pageindex='+this.pageindex+'&pagesize='+this.pagesize);
  
     if(res.Res==1){ 
        
         
              if(res.data.arrays.length>0){

        this.dataobj.arr.arrays= this.dataobj.arr.arrays.concat(res.data.arrays);
         //避免下拉到原来的位置
