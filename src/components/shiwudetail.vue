<template>
 ...
 <view class="bbox xycenter tubiao">
    ...
    <view>
            <view class="mes">
        <view>接收人：<text class="col">{{childdata.NickName}}</text></view>
 ...
 <view>合同名称：<text class="col">{{childdata.CompactName}}</text></view>
      ...
      <view class="fujian bbox">
          ...
          <view class="rf">
            <view v-for="(item, itemIndex) in childdata.arrayF" :key="itemIndex"  @click="download(item.fileurl,item.filetype)" >
                   {{item.filename}}
              </view>
          </view>
