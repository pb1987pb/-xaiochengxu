    ...
    <view>
      <view class="mes">
        <view>申请人：<text class="col">{{childdata.NickName}}</text></view>
            ...
            <view>
              <view>项目收款进度：<text class="col">{{childdata.PaymentStatus}}%</text></view>
             <view>当前预算余额：<text class="col">￥{{childdata.CurBudget}}</text></view>
           <view>申请额外预算总金额<text class="col">￥{{childdata.SumMoney}}</text></view>
           <view>更改前毛利率：<text class="col">{{childdata.OldGrossProfitMargin}}%</text></view>
           <view>更改后毛利率：<text class="col">{{childdata.NewGrossProfitMargin}}%</text></view>
           <view>各项额外预算：</view>

              <view v-if="childdata.arrayChailuCount">
  <view>差旅:</view>
           <view class="bbox"  v-for="(item, itemIndex) in childdata.arrayChailu" :key="itemIndex" >
               <view class="vie">{{item.AttrName}}：<text class="col">￥{{item.Value}}</text></view>
               <view>税点：<text class="col">{{item.Tex}}%</text></view>
           </view>
              </view>
         
      <view v-if="childdata.arrayDahuoCount">
  <view>大货：</view>
           <view class="bbox"  v-for="(item, itemIndex) in childdata.arrayDahuo" :key="itemIndex" >
               <view class="vie">{{item.AttrName}}：<text class="col">￥{{item.Value}}</text></view>
               <view>税点：<text class="col">{{item.Tex}}%</text></view>
           </view>
              </view>

    <view v-if="childdata.arrayWuliuCount">
  <view>物流：</view>
           <view class="bbox"  v-for="(item, itemIndex) in childdata.arrayWuliu" :key="itemIndex" >
               <view class="vie">{{item.AttrName}}：<text class="col">￥{{item.Value}}</text></view>
               <view>税点：<text class="col">{{item.Tex}}%</text></view>
           </view>
              </view>
    <view v-if="childdata.arrayYangbanCount">
  <view>样板：</view>
           <view class="bbox"  v-for="(item, itemIndex) in childdata.arrayYangban" :key="itemIndex" >
               <view class="vie">{{item.AttrName}}：<text class="col">￥{{item.Value}}</text></view>
               <view>税点：<text class="col">{{item.Tex}}%</text></view>
           </view>
              </view>
      ...
      <view class="fujian bbox">
          ...
          <view class="rf" v-if="childdata.array">
